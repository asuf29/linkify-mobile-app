...
function ViewFollowingScreen(props) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        endpoint = username ? "/profiles/user_followings" : "/profiles/current_followings"
        ...
        setUsers(response.data.data.followings);
  ...
  return (
    <View style={tw`flex-1 items-center mt-20`}>
      <Text style={tw`font-bold text-lg mb-4`}>All Following</Text>
      <FlatList
        data={users}
